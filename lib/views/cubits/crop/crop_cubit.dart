...
import 'package:farmtracker/views/cubits/crop/crop_state.dart';
...
class CropCubit extends Cubit<CropState> {
  ...
  Future<void> carregarCrops() async {
    ...
    result.fold((crops) => emit(CropListLoaded(crops)), (_) => emit(const CropErro('Falha ao carregar crops.')));
  }
  ...
  Future<void> obterPorName(String name) async {
    ...
    result.fold((crops) => emit(CropListLoaded(crops)), (_) => emit(const CropErro('Falha ao buscar crop por nome.')));
  }
  ...
  Future<void> obterPorId(String id, String orgOwner) async {
    ...
    result.fold((crop) => emit(CropLoaded(crop)), (_) => emit(const CropErro('Crop não encontrado.')));
  }
